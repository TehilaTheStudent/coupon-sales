...
public class JwtUtils implements Filter {
    ...
    public String generateToken(Credentials credentials, Role role) {
        ...
        return createToken(claims, credentials.getEmail(), role);
    }

    private String createToken(Map<String, Object> claims, String subject , Role role) {
        return Jwts.builder().setClaims(claims).setSubject(subject).setIssuer(role.name()).setIssuedAt(new Date())
    ...
    private boolean validateToken(String jwt) throws ServletException {
        ...
        try {
            ...
            newToken = generateToken(new Credentials(claims.getSubject(), null), Role.valueOf(claims.getIssuer()));
            removeToken(jwt);
        } catch (Exception e) {
            return false;
        }
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        //Check if there is an options call before the actual call
        if (HttpMethod.OPTIONS.name().equalsIgnoreCase(request.getMethod()) ){
            ...
            if (!request.getRequestURI().contains("login")&&(request.getRequestURI().contains("admin")||request.getRequestURI().contains("customer")||request.getRequestURI().contains("company"))){
                //Check if token startsWith Bearer and != null
                //for those who need authontication
                ...
                //for those who dont need authontication
                chain.doFilter(req, response);
            }
        }
    }
    ...
    private boolean check(String requestURI, String jwt) {
        ...
        try {
            ...
            if(issuer.equals("administrator"))
                issuer = "admin";
            bool = requestURI.contains(issuer.substring(0,issuer.length()-5));
        } catch (Exception e) {
            return false;
        }
        return bool;
